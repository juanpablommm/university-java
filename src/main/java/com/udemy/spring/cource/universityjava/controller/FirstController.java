...
public class FirstController {
    ...
    * completo para poder aplicar el method save y guardar en la base de datos*/
    ...
    /* al especificar el path tambien debemos especificar el nombre de la varaible que viene desde la template por
    ...
    * esto es con el fin que se inicilice el parametro de tipo Person que tiene nuestro end-point, este solo se inicilizara con el id
    ...
    * de datos y no un nuevo insert*/
    ...
    /*dentramos anuestro end-point, a travez del path /delte y especificamos dentro de las llaves el
    * nombre del path param que recivimos de la vista, de nuestra template, el cual con ayuda de spring al momento
    * de relizarce la inyeccion de dependencias y otener una instancia de nuestro parametro  de tipo Person, se
    * se trendra el el valor del atributo id, y finalmanete haremos la eliminacion de ese registro a travez de nuestro
    * service pasandole como parametro el el objeto persona*/
    ...
    */
    ...
    /*para nosotrso recivir un Query Params que venga desde la vista simplemente, ya no utlizamos la sintaxis de
    * {} y el nombre del parametro como lo haciamos por path params, sino que vasta con utilizar el path por el que
    * se le hace el llamado al end-point desde la vista, y automaticamente spring realizara la instancia de nuesto objeto
    * person setiandole el valor del atributo que venga por Query params, eso si tiene que ser al igual que por path param
    * el mismo nombre que el atributo del objeto*/
    @GetMapping("/delete")
    public String dellete(Person person){
        this.service.deletePerson(person);
        return "redirect:/";
